class Game {
  ...
  start() {
    ...
    //crear grupos de fuels y powerCoins
    fuels = new Group()
   ...
   this.addSprite(powerCoins, 7, powerCoinImage, 0.09)
  }
  

  //CREAR METODO ADDSPRITE
 addSprite(spriteGroup, numberOfSprites, spriteImage, scale){
  //FOR ES UN BUCLE QUE LO QUE ES HACE ES REPETIRSE
  ...
  handleFuel(index){
    cars[index - 1].overlap(fuels, function(collector, 
      collected){
        player.fuel = 185;
        collected.remove()
      })
 ...
 handlePowerCoins(index){
    cars[index - 1].overlap(powerCoins, function(collector, 
      collected){
        player.score += 20;
        player.update()
        collected.remove()
